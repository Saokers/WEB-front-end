...
export default {
  ...
  methods: {
    ...
    productDetailList(productId) {
      var vm = this;
      vm.$get(vm.API.API_URL_CATALOG_PRODUCT_DETAILS + "/" + productId).then(
        res => {
          if (res.errorCode == 0) {
    ...
    getSkuList(productId) {
      var vm = this;
      vm.$get(vm.API.API_URL_CATALOG_PRODUCT_SKU + "/" + productId).then(
        res => {
              ...
              for (var item in res.data) {
                for (var option in res.data[item].optionValuesList) {
                  for (var sku in vm.skuList) {
                    var optionValueKeyGroup = vm.skuList[
                      sku
                    ].optionValueKeyGroup.split("_");
                    for (var optionGItem in optionValueKeyGroup) {
                      ...
                      if (optionId == optionValueKeyGroup[optionGItem]) {
                        optionShow.push(
                          res.data[item].optionValuesList[option]
                        );
                      }
                    }
                  }
    ...
    optionSelected(optionId) {
      ...
      if (this.skuList) {
        for (var sku in this.skuList) {
          var optionValueKeyGroup = this.skuList[sku].optionValueKeyGroup.split(
            "_"
          );
          for (var optionGItem in optionValueKeyGroup) {
            if (optionId == optionValueKeyGroup[optionGItem]) {
    ...
    loadreviewsList(productId) {
      var vm = this;
      vm.$get(vm.API.API_URL_CATALOG_SHOW_REVIEW + "/" + productId).then(
        res => {
          if (res.errorCode == 0) {
    ...
    cartOrOrder(flag) {
      ...
      if (this.optionList.length != 0) {
        if (this.params.optionValueKeyGroup) {
          ...
          this.params.jsonData = this.getJsonData(
            this.params.optionValueKeyGroup
          );
        } else {
      ...
      for (var key in this.skuList) {
        if (
          this.skuList[key].optionValueKeyGroup ==
          this.params.optionValueKeyGroup
        ) {
          ...
          vm.$post(vm.API.API_URL_CATALOG_ADD_REVIEW, vm.reviews).then(res => {
            if (res.errorCode == 0 && res.data) {
              if (res.data.success) {
                ...
                this.reviews.content=null;
                this.reviews.stars=null;
